...
void cameraHandler::writeServ(int ang) {
  serv.write(750 + ((long)1000 * (long)ang) / (long)180);
}

void cameraHandler::evaluateData() {
...
void cameraHandler::attach(unsigned char _servoPin, unsigned char _sensorPin) {
  servoPin = _servoPin;
